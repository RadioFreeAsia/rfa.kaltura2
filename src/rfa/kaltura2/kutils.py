...
def uploadThumbnail(context, client=None):
    """This uploads the custom thumbnail to Kaltura
        """
    usingEntitlements = False

    #get the file field and....

    field = context.custom_thumbnail
    if field.filename is None:
        return 1 #there is no file.
    ...
    if client is None:
        (client, session) = kconnect()
    entry_id = context.KalturaObject.getId()
    thumbAsset = client.thumbAsset.addFromImage(entry_id, tempfh)
...
def setModerationStatus(context, status, client=None):
    ...
    vid_object = getattr(context, 'KalturaObject', None)
    if vid_object:
        vid_id = vid_object.getId()
    else:
        vid_id = context.entryId
    if status in (2,):
        updateEntry = client.media.approve(vid_id)
    elif status in (3,1,5):
        updateEntry = client.media.reject(vid_id)
